...
import stockAlert from "../../../../assets/image/stockAlert.png";
...
export default function InventoryContainer({ newProductAdded }) {
  ...
  useEffect(() => {
    ...
    if (lowQuantityProductsData.length > 0) {
      ...
      setLowQuantityProducts(
        lowQuantityProductsData.map((product) => product.nombre).join(", ")
      );
    } else {
      setLowQuantityAlert(false);
    }
  ...
  }, [consultedMonth, allProductsData]);
